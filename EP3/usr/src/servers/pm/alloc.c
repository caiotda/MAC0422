 ...
 */
  register struct hole *hp, *prev_ptr, *largest;
  ...
  do {
    ...
    if (worst_fit) {
        ...
        old_base = largest->h_base;
        largest->h_base += clicks;
        largest->h_len -= clicks;
        
        if(largest->h_base > high_watermark)
          high_watermark = largest->h_base;

        if(largest->h_len == 0) del_slot(prev_ptr, largest);
        return(old_base);
    }
    else {
        prev_ptr = NIL_HOLE;
